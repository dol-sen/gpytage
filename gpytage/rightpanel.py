...
rightselection.set_mode(Gtk.SelectionMode.MULTIPLE)
...
# rightview.enable_model_drag_source(Gdk.BUTTON1_MASK, [('text/plain', 0, 0)], Gdk.ACTION_DEFAULT | Gdk.ACTION_MOVE)
# rightview.enable_model_drag_dest([('text/plain', 0, 0)], Gdk.ACTION_DEFAULT)
...
def __handleKeyPress(widget, event):
    ...
    if event.keyval == Gdk.keyval_from_name("c"):
        if (modifiers & event.state) == Gdk.ModifyerType.CONTROL_MASK:
            clipboard.copyToClipboard(rightview)
    # paste
